...
function PointAtSun{
    ...
    wait until (vang(sun:Position, ship:facing:vector) < 1).
    unlock steering.
    sas on.
}.

function ChangeApoapsis{
    declare parameter desiredApoapsis.
    declare parameter shipVectors to GetShipVectors().
    declare parameter orbitBody to ship:body.

    local radiusAtBurn to GetRadiusFromTrueAnomaly(0, shipVectors).
    local transferOrbitSemiMajorAxis to (radiusAtBurn + desiredApoapsis) / 2.
    local requiredSpeedAfterBurn to sqrt(orbitBody:mu *((2/ radiusAtBurn)-(1/transferOrbitSemiMajorAxis))).
...
function ChangePeriapsis{
    declare parameter desiredPeriapsis.
    declare parameter shipVectors to GetShipVectors().
    declare parameter orbitBody to ship:body.

    local radiusAtBurn to GetRadiusFromTrueAnomaly(180, shipVectors).
    local transferOrbitSemiMajorAxis to (radiusAtBurn + desiredPeriapsis) / 2.
    local requiredSpeedAfterBurn to sqrt(orbitBody:mu *((2/ radiusAtBurn)-(1/transferOrbitSemiMajorAxis))).
    ...
    CreateManoeuvreNodeAtTrueAnomaly(180, burnVector, 0, shipVectors).
    RunNodeBurn().
}
